...
fn looks_like_windows_1252_punctuation(bytes: &[u8]) -> bool {
    ...
    for &byte in bytes {
        // If we see bytes beyond the 0x80-0x9F range (excluding ASCII), this isn't just
        // punctuation
        if byte >= 0xA0 {
            return false;
        }
...
mod tests {
    ...
    fn test_cp1251_russian_text() {
        // Cyrillic text emitted by PowerShell/cmd in CP1251 should decode cleanly.
    ...
    #[test]
    fn test_empty_input() {
        assert_eq!(bytes_to_string_smart(&[]), String::new());
    }
}
